...
const materials = [
  ...
  { id: "ALB003", name: "Brushed Copper", image: "/materials/Brush3.webp", category: "Anodised" },
...
/* ─── HERO REMOVED ── grid starts directly after navbar ─────── */
...
const SwatchCard = ({
  ...
  return (
    ...
    >
      {/* Image container */}
      <div className="relative w-full aspect-square overflow-hidden rounded-2xl shadow-[0_6px_24px_rgba(0,0,0,0.08)] border border-black/5">
        <img
          src={mat.image}
          alt={mat.name}
        ...
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className="absolute inset-0 flex items-center justify-center bg-black/55 backdrop-blur-[2px] rounded-2xl"
            >
              <motion.span
                ...
                exit={{ y: 4, opacity: 0 }}
                transition={{ duration: 0.2 }}
                className="text-[10px] tracking-[0.22em] uppercase font-semibold text-white border border-white/30 px-4 py-1.5 rounded-full bg-white/10"
              >
                {mat.category}
              </motion.span>
            </motion.div>
      ...
      <div className="mt-3 px-0.5">
        ...
        <p className="text-[10px] font-medium text-[#0a4b7c]/70 tracking-wider mt-0.5">
          {mat.id.trim()}
        </p>
      </div>
    </motion.div>
...
const GridSection = () => {
  ...
  const filtered =
    active === "All" ? materials : materials.filter((m) => m.category === active);
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-black/[0.02] rounded-full blur-[100px] pointer-events-none" />
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto">
        ...
        >
          {categories.map((cat) => (
            <motion.button
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.97 }}
              className={`relative overflow-hidden rounded-full px-5 py-2.5 text-xs font-semibold tracking-wide transition-all duration-300 ${
                active === cat
        ...
        <motion.p
          ...
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="text-xs text-gray-400 font-medium mb-8 tracking-wide"
        >
          Showing{" "}
          <span className="text-[#0a4b7c] font-bold">{filtered.length}</span>{" "}
          {filtered.length === 1 ? "finish" : "finishes"}
        ...
        <AnimatePresence mode="wait">
          ...
          >
            {filtered.map((mat, i) => (
              <SwatchCard key={mat.id + mat.name + i} mat={mat} index={i} isInView={isInView} />
            ))}
          </motion.div>
        </AnimatePresence>
        ...
        >
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            ...
            <div className="max-w-lg">
              <h4 className="text-2xl md:text-3xl font-medium text-[#1A1A1A] mb-3 tracking-tight">
                Can't find what you're{" "}
                <span className="text-[#0a4b7c]">looking for?</span>
              </h4>
              <p className="text-sm text-[#6B7280] leading-relaxed font-light">
                We offer bespoke colour matching and custom finishes engineered
                specifically for your architectural requirements. Download the
                full chart or reach out to our team.
              </p>
            </div>

            {/* Two Buttons */}
            <div className="flex flex-wrap items-center gap-4 shrink-0">
              {/* Button 1: Download Color Chart */}
              <a
                href="/Alubond Color Chart A3 .pdf"
                download="Alubond Color Chart"
                className="group relative inline-flex items-center justify-center gap-2.5 px-7 py-3.5 bg-[#0a4b7c] text-white rounded-full overflow-hidden transition-all duration-300 hover:shadow-xl hover:shadow-[#0a4b7c]/25 hover:-translate-y-1"
              >
                <Download size={15} className="relative z-10" />
                <span className="relative z-10 text-[12px] tracking-wider font-bold uppercase">
              ...
              <Link
                to="/contact"
                className="group relative inline-flex items-center justify-center gap-2.5 px-7 py-3.5 bg-white border border-[#1A1A1A] text-[#1A1A1A] rounded-full overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1"
              >
                <Mail size={15} className="relative z-10 transition-colors duration-300 group-hover:text-white" />
                <span className="relative z-10 text-[12px] tracking-wider font-bold uppercase transition-colors duration-300 group-hover:text-white">
                  Contact Our Team
                </span>
                <div className="absolute inset-0 bg-[#1A1A1A] scale-x-0 origin-left group-hover:scale-x-100 transition-transform duration-500 ease-out rounded-full" />
              </Link>
